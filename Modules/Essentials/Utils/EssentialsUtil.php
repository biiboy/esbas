...
class EssentialsUtil extends Util
{
    ...
    public function getTotalTelatMenit(
        $user_id,
        $business_id,
        $month = null,
        $year = null
        ...
        $telat_sum = $query->select(DB::raw('SUM(COALESCE(telat_menit, 0)) as total_minutes'))->first();
        $total_telat_menit = ! empty($telat_sum?->total_minutes) ? $telat_sum->total_minutes : 0;

        return $total_telat_menit;
    }

    public function getTotalLemburMenit(
        $user_id,
        $business_id,
        $month = null,
        $year = null
        ...
        $telat_sum = $query->select(DB::raw('SUM(COALESCE(lembur_menit, 0)) as total_minutes'))->first();
        $total_lembur_menit = ! empty($telat_sum?->total_minutes) ? $telat_sum->total_minutes : 0;

        return $total_lembur_menit;
    }
    ...
    public function checkUserShift($user_id, $settings, $clock_in_time = null)
    {
        ...
        foreach ($user_shifts as $shift) {
            ...
            // if ((! empty($shift->start_time)) || $shift->type == 'flexible_shift') {
    ...
    public function checkUserShiftOut($user_id)
    {
        $shift_id = null;
        $shift_date = \Carbon::now()->format('Y-m-d'); // Mengambil tanggal saat ini
        $day_string = strtolower(\Carbon::now()->format('l')); // Nama hari dalam lowercase

        // Ambil shift berdasarkan `user_id` dan tanggal
        $user_shifts = EssentialsUserShift::join('essentials_shifts as s', 's.id', '=', 'essentials_user_shifts.essentials_shift_id')
            ->where('user_id', $user_id)
            ->where('start_date', '<=', $shift_date)
            ->where(function ($q) use ($shift_date) {
                $q->whereNull('end_date')
                    ->orWhere('end_date', '>=', $shift_date);
            })
            ->select('essentials_user_shifts.*', 's.holidays', 's.start_time', 's.end_time', 's.type')
            ->get();
        ...
        foreach ($user_shifts as $shift) {
            ...
            // Lewati jika hari ini adalah hari libur
            if (is_array($holidays) && in_array($day_string, $holidays)) {
                continue;
            }

            // Kembalikan ID shift jika cocok dengan user_id dan tanggal
            return $shift->essentials_shift_id;
        }

        return $shift_id; // Mengembalikan null jika tidak ada shift yang cocok
    }
    ...
    public function clockin($data, $essentials_settings)
    {
        ...
        if ($count == 0) {
            // cek keterlambatan
            $user_shift = Shift::find($shift);
            $shift_start_time = \Carbon::parse($user_shift->start_time); // Time the shift starts

            // Get current time for clock in
            $clock_in_time = $data['clock_in_time'];

            // Calculate lateness in minutes
            $lateness_in_minutes = 0;
            if ($clock_in_time->greaterThan($shift_start_time)) {
                $lateness_in_minutes = $shift_start_time->diffInMinutes($clock_in_time);
            }

            $data['telat_menit'] = $lateness_in_minutes;
    ...
    public function clockout($data, $essentials_settings)
    {
        ...
        if (! empty($clock_in)) {
            ...
            // cek lembur
            $shift = $this->checkUserShiftOut($data['user_id']);
            $user_shift = Shift::find($shift);
            $shift_end_time = \Carbon::parse($user_shift->end_time); // Time the shift ends

            // Get current time for clock in
            $clock_out_time = \Carbon::parse($data['clock_out_time']);

            // Calculate lembur in minutes
            $lembur_in_minutes = 0;
            if ($clock_out_time->greaterThan($shift_end_time)) {
                $lembur_in_minutes = $shift_end_time->diffInMinutes($clock_out_time);
            }

            $clock_in->lembur_menit = $lembur_in_minutes;
            ...
            $clock_in->clock_out_latitude = $data['clock_out_latitude'] ?? '';
            $clock_in->clock_out_longitude = $data['clock_out_longitude'] ?? '';
            $clock_in->save();

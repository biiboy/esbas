...
class ReportController extends Controller
{
    ...
    public function getProdukKurangLaku(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        ...
        $products = $this->productUtil->getProdukKurangLaku($business_id, $filters);
        ...
        return view('report.produk_kurang_laku')
            ->with(compact('chart', 'categories', 'brands', 'units', 'business_locations', 'filters'));
    }

    public function getProdukTidakBergerak(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');

        if (request()->ajax()) {

            $filters['brand_id'] = $request->get('brand_id', null);
            $filters['location_id'] = $request->get('location_id', null);
            ...
            // Ambil data produk yang tidak bergerak
            ...
            $transactions = $transactions->pluck('product_id')->toArray();
            ...
            // filter untuk menghilangkan produk yang sudah ada di list transaksi
            if (!empty($transactions)) {
                $query->whereNotIn('products.id', $transactions);
            }

            if (! empty($filters['location_id'])) {
                if ($permitted_locations == 'all' || in_array($filters['location_id'], $permitted_locations)) {
                    $query->whereHas('product_locations', function ($query) use ($filters) {
                        $query->where('product_locations.location_id', '=', $filters['location_id']);
                    });
                }
            } else {
                if ($permitted_locations != 'all') {
            ...
            return Datatables::of($products)
                ...
                ->rawColumns(['image', 'product', 'selling_price', 'purchase_price', 'category', 'current_stock'])
                ->make(true);
        }
        ...
        return view('report.produk_tidak_bergerak')
